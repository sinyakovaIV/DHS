...
begin
  if not CheckExcelInstalled('Excel.Application') then
    Application.MessageBox(PChar('Необходимо установить Microsoft Excel.'), 'Ошибка', MB_ICONERROR);
//  else
...
begin
  try
    ...
    // хотя и при запущенном excel'е тоже !ИНОГДА! операция бывает недоступна
...
begin
  try
    ADest := GetActiveOleObject(AValue);  // операция недоступна
    Result := true;
  except
    Result := false;
  end;
