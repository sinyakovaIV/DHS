...
begin
  ...
  try
    ...
    CurAcademicYear := AcademicYear[cmbAcademicYear.ItemIndex]
      .ValueStrByName('NAME');
    Memo1.Clear;
    sline := 'Подробное расписание педагога:   ' + PedSurname + '   ' +
      CurAcademicYear + ' уч.год.    Дата отчета:  ' + DateToStr(Date);
    ...
    sline := '';
    sline := '№' + #9 + 'Д/нед' + #9 + 'Время' + #9 + 'Группа / учащийся' + #9 +
      'Прогр.' + #9 + 'Уровень' + #9 + 'Действ с' + #9 + 'Действ по' + #9 + 'каб.';
    Memo1.Lines.Add(sline);
...
//      ' - Подробное расписание на  ' + DateToStr(Date);
    ExcelFileName := CurAcademicYear + PedSurname +
      ' - Подробное расписание на  ' + IntToStr(DayOf(Now)) + ' ' + IntToStr(MonthOf(Now));
      //
