...
procedure TfReport_PedWorkLoad.bExportToExcelClick(Sender: TObject);
var
...
begin
  ...
  try
    ...
    Sheet.PageSetup.CenterFooter :=
      ...
      DateToStr(DateTimePicker1.Date) + '&B  _____________ Подпись' +  'Стр. &С из &К';
...
//Sheet.PageSetup.CenterFooter := 'Стр. &С из &К';
    ...
    // в отчете нагрузки не должно быть отчисленных и подумать насчет типа учебной группы - все выводить или нет...
    ...
    ExcelApp.WorkBooks[1].WorkSheets[1].Columns[1].ColumnWidth:= 20;
    ExcelApp.WorkBooks[1].WorkSheets[1].Columns[2].ColumnWidth:= 11;
    ExcelApp.WorkBooks[1].WorkSheets[1].Columns[3].ColumnWidth:= 7;
    ExcelApp.WorkBooks[1].WorkSheets[1].Columns[4].ColumnWidth:= 5;
