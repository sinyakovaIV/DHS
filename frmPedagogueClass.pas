...
type
  ...
  public
    ...
    property IDCurAcademicYear: integer read FIDCurAcademicYear write SetIDCurAcademicYear;
  end;
...
begin
  ...
  if fPedagogueClassEdit.ModalResult = mrOk then
  begin
...
procedure TfPedagogueClass.SetIDCurAcademicYear(const Value: integer);
begin
  if FIDCurAcademicYear <> Value then
    FIDCurAcademicYear := Value;
end;

procedure TfPedagogueClass.SetIDPedagogue(const Value: integer);
begin
...
var
  in_id_education_program, in_id_pedagogue, in_id_academic_year: integer;
begin
  ...
  if Assigned(ClassMembersList) then
    FreeAndNil(ClassMembersList);
  ClassMembersList := Kernel.GetChildListClassMember(in_id_education_program,
    in_id_pedagogue, in_id_academic_year);
  Kernel.GetLVChildListClassMember(lvMainClassList, ClassMembersList);
