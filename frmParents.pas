...
begin
  ...
  // после сохранялки сделать, чтобы неперескакивало на первую запись в LV!!!!!!
  ShowParentList(IDChild); // обновляю отображалку
end;
...
// кажется нескладушка - сверить с детской формой напр. где идет заливка пола
...
begin
  ...
  // а если нет записей, то может деактивировать кнопку "изменить" и "удалить"
  begin
